...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0], load_coarse=False):
        ...
        self.val_camera = FourDGSdataset(scene_info.val_cameras, args, dataset_type)
        ...
        if scene_info.point_cloud_second is not None:
            # scene_info.point_cloud = scene_info.point_cloud_second
            xyz_max = scene_info.point_cloud_second.points.max(axis=0)
            xyz_min = scene_info.point_cloud_second.points.min(axis=0)
            if args.add_points:
                print("add points.")
                # breakpoint()
                scene_info = scene_info._replace(point_cloud=add_points(scene_info.point_cloud_second, xyz_max=xyz_max, xyz_min=xyz_min))
            ...
            if self.loaded_iter:
                ...
                self.gaussians.create_from_pcd(scene_info.point_cloud_second, self.cameras_extent, self.maxtime)
        else:
...
class Scene2gs_mixed:
    ...
    def __init__(self, args: ModelParams, gaussians: GaussianModel, load_iteration=None, shuffle=True,
                 resolution_scales=[1.0], load_coarse=False, gaussians_second=None):
        """b
        :param path: Path to colmap scene main folder.
        """
        self.model_path = args.model_path
        ...
        if load_iteration:
            # if load_iteration == -1:
            self.loaded_iter = searchForMaxIteration(os.path.join(load_iteration, "point_cloud"),-1)
            # else:
            #     self.loaded_iter = load_iteration
            print("Loading trained model at iteration {}".format(self.loaded_iter))
        ...
        if args.add_points:
            print("add points.")
            # breakpoint()
###################### use the third point cloud to set deformation
            scene_info = scene_info._replace(
                point_cloud=add_points(scene_info.point_cloud_third, xyz_max=xyz_max, xyz_min=xyz_min))
        self.gaussians._deformation.deformation_net.set_aabb(xyz_max, xyz_min)
