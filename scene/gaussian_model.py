...
class GaussianModel_dynamic:
    ...
    def __init__(self, sh_degree: int, args):
        ...
        # self.feature_mask = torch.empty(0)
    ...
    def get_covariance(self, scaling_modifier=1):
        """
        Computes the covariance matrix for the Gaussians.

        Args:
            scaling_modifier (float): A modifier for the scaling.

        Returns:
            torch.Tensor: The computed covariance matrix.
        """
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float, time_line: int):
        ...
        Args:
            ...
            time_line (int): The current time step.
        """
        self.spatial_lr_scale = spatial_lr_scale
        # breakpoint()
        fused_point_cloud = torch.tensor(np.asarray(pcd.points)).float().cuda()
        fused_color = RGB2SH(torch.tensor(np.asarray(pcd.colors)).float().cuda())
        features = torch.zeros((fused_color.shape[0], 3, (self.max_sh_degree + 2) ** 2)).float().cuda()
        ...
        features_t = torch.ones((fused_point_cloud.shape[0], 1), device="cuda") * 0.5
        ...
        # self.feature_mask = nn.Parameter(features_t.contiguous().requires_grad_(True))
    ...
    def reset_opacity(self):
        ...
        optimizable_tensors = self.replace_tensor_to_optimizer(opacities_new, "opacity")
        self._opacity = optimizable_tensors["opacity"]
    def reset_sh_partially_first(self):
        # opacities_new = self._features_rest
        sh_new_direct = self._features_dc
        sh_new = self._features_rest
        sh_new_replace = torch.clamp(
            sh_new, max=torch.abs(torch.tensor(0.001)).item()
        )
        sh_new_replace_direct = torch.clamp(
            sh_new_direct, max=torch.abs(torch.tensor(0.001)).item()
        )
        opacities_mask = np.array(list(range(0, self.get_opacity.shape[0])))
        opacities_mask_select = np.random.choice(opacities_mask, int(opacities_mask.shape[0] * 0.2), replace=False)
        sh_new[opacities_mask_select, :16, :] = sh_new_replace[opacities_mask_select, :16, :]
        sh_new_direct[opacities_mask_select, :,:] = sh_new_replace_direct[opacities_mask_select, :,:]
        optimizable_tensors = self.replace_tensor_to_optimizer(sh_new, "f_rest")
        self._features_rest = optimizable_tensors["f_rest"]
        optimizable_tensors = self.replace_tensor_to_optimizer(sh_new_direct, "f_dc")
    ...
    def load_ply(self, path):
        ...
        assert len(extra_f_names) == 3 * (self.max_sh_degree + 2) ** 2 - 3
        features_extra = np.zeros((xyz.shape[0], len(extra_f_names)))
        for idx, attr_name in enumerate(extra_f_names):
            features_extra[:, idx] = np.asarray(plydata.elements[0][attr_name])
        # Reshape (P,F*SH_coeffs) to (P, F, SH_coeffs except DC)
        features_extra = features_extra.reshape((features_extra.shape[0], 3, (self.max_sh_degree + 2) ** 2 - 1))
    ...
    def prune_points(self, mask, reset_prob=False):
        ...
        if reset_prob:
            self.max_radii2D = torch.zeros_like(self.max_radii2D).cuda()
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling,
                              new_rotation, new_deformation_table, new_max_radii2D):
        d = {"xyz": new_xyz,
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation,
                                   new_deformation_table, new_max_radii2D)
    ...
    def prune(self, max_grad, min_opacity, extent, max_screen_size, run_pruning_prob=False):
        prune_mask = (self.get_opacity < min_opacity).squeeze()
        ## use foregound points proba

        use_foregound_op = True
        throw_out = False
        if use_foregound_op and run_pruning_prob and max_screen_size:
            foregound_op_too_small = self.max_radii2D < (min_opacity * 0.1)
            prune_mask = torch.logical_or(prune_mask, foregound_op_too_small)
            print("foregound_op_too_small: ", foregound_op_too_small.sum())
        if throw_out and max_screen_size:
            opacities_new = self.get_opacity
            opacities_new_replace = inverse_sigmoid(
                torch.min(self.get_opacity, torch.ones_like(self.get_opacity) * 0.01))
            opacities_mask = np.array(list(range(0, self.get_opacity.shape[0])))
            opacities_mask_select = np.random.choice(opacities_mask, int(opacities_mask.shape[0] * 0.1), replace=False)
            prune_mask[opacities_mask_select] = True
            print("throw out: ", opacities_mask_select.shape[0])

        if run_pruning_prob:
            self.prune_points(prune_mask, reset_prob=True)
        else:
...
class GaussianModel:
    ...
    def setup_functions(self):
        ...
        (Same as in `GaussianModel_dynamic`)
        """
        def build_covariance_from_scaling_rotation(scaling, scaling_modifier, rotation):
            L = build_scaling_rotation(scaling_modifier * scaling, rotation)
            actual_covariance = L @ L.transpose(1, 2)
    ...
    def save_ply(self, path):
        ...
        el = PlyElement.describe(elements, 'vertex')
        PlyData([el]).write(path)
    def reset_sh_partially(self):
        # opacities_new = self._features_rest
        sh_new = self._features_rest
        sh_new_dc = self._features_dc
        sh_new_replace_dc = torch.clamp(
            sh_new_dc, max=torch.abs(torch.tensor(0.001)).item()
        )
        ...
        opacities_mask = np.array(list(range(0, self.get_opacity.shape[0])))
        opacities_mask_select = np.random.choice(opacities_mask, int(opacities_mask.shape[0] * 0.1), replace=False)
        sh_new[opacities_mask_select] = sh_new_replace[opacities_mask_select]
        sh_new_dc[opacities_mask_select] = sh_new_replace_dc[opacities_mask_select]

        optimizable_tensors = self.replace_tensor_to_optimizer(sh_new, "f_rest")
        self._features_rest = optimizable_tensors["f_rest"]
        optimizable_tensors = self.replace_tensor_to_optimizer(sh_new_dc, "f_dc")
        self._features_dc = optimizable_tensors["f_dc"]
    def reset_sh_partially_second(self):
        # opacities_new = self._features_rest
        sh_new = self._features_rest
        sh_new_dc = self._features_dc
        sh_new_replace_dc = torch.clamp(
    ...
    def reset_opacity(self):
        """
        Resets the opacity of Gaussians to a small value.
        """
        opacities_new = inverse_sigmoid(torch.min(self.get_opacity, torch.ones_like(self.get_opacity) * 0.01))
        optimizable_tensors = self.replace_tensor_to_optimizer(opacities_new, "opacity")
        self._opacity = optimizable_tensors["opacity"]
    ...
    def prune(self, max_grad, min_opacity, extent, max_screen_size):
        ...
        if throw_out and max_screen_size:
            ...
            opacities_mask_select = np.random.choice(opacities_mask, int(opacities_mask.shape[0] * 0.1), replace=False)
            prune_mask[opacities_mask_select] = True
            print("throw out: second", opacities_mask_select.shape[0])

        if max_screen_size and False:
            # big_points_vs = self.max_radii2D > max_screen_size
            soft_prune = True
            if soft_prune:
                percentile_80 = torch.quantile(self.get_scaling.max(dim=1).values, 0.8)

                big_points_vs = self.get_scaling.max(dim=1).values > max(percentile_80.item(), 0.1 * extent)
                prune_mask = torch.logical_or(prune_mask, big_points_vs)
            else:
                big_points_vs = self.get_scaling.max(dim=1).values > 0.1 * extent
